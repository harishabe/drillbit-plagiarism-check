...
import { GetExtremeRefData } from "../../redux/action/super/SuperAdminAction";
import { DownloadCsv } from "../../redux/action/common/Submission/SubmissionAction";
import { UploadFileDataClear } from "../../redux/action/admin/AdminAction";
...
import { platform, getItemSessionStorage } from "../../utils/RegExp";
...
const Pro = ({
  ...
  const handleDownloadYesWarning = () => {
    DownloadCsv(
      BASE_URL_SUPER + END_POINTS.CONSORTIUM_CSV_DOWNLOAD + "proConsortiumCsv",
      DOWNLOAD_CSV.CONSORTIUM_PRO
    );
    setShowModal(false);
  };
    ...
    <>
      <Grid container spacing={2}>
        ...
        <Grid item md={0.5} xs={0.5}>
          <DownloadButton>
            {extremeData?.length > 0 && isLoadingDownload ? (
              <SkeletonContainer>
...
const mapDispatchToProps = (dispatch) => {
  return {
    GetExtremeRefData: (url, paginationValue) =>
      dispatch(GetExtremeRefData(url, paginationValue)),

...
const ExtremeForm = ({
    ...
    const onSubmit = (data) => {
        if (editOperation) {
            ...
            EditAccount(END_POINTS.CONSORTIUM_EXTREME_LICENSES + '/' + editData?.lid, END_POINTS.CONSORTIUM_EXTREME, requestData);
        } else {
            let DetailedData = {
                ...data,
                'endDate': convertDate(data?.endDate),
            ...
            let requestData = Object.entries(DetailedData).reduce((newObj, [key, value]) => (value == '' ? newObj : (newObj[key] = value, newObj)), {});
            CreateAccount(END_POINTS.CONSORTIUM_EXTREME_LICENSES, END_POINTS.CONSORTIUM_EXTREME, requestData);
        }
    };

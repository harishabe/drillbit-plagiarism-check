...
const TicketResponses = ({ myTicketsData, myTicketsIdData, GetTicketData, CreateTicketResponse, isLoadingResponse, GetTicketIdData, CloseTicket }) => {
  ...
  useEffect(() => {
    if (router.isReady) {
      const url = BASE_URL_SUPER + END_POINTS.ADMIN_TICKET_DETAILS + "/" + router.query.ticketId ;
      GetTicketIdData(url);
    }
  }, [router.isReady ]);
  ...
  const formatDate = (messageDate) => {
    ...
    return (
      <React.Fragment>
        ...
        <Grid item md={3} xs={3}>
        <Button onClick={closeTicket} variant="contained" color="primary" className={classes.closeTicket}>
      Close ticket
    </Button>
        </Grid>
      </Grid>

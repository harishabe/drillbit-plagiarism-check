    ...
    <React.Fragment>
      ...
      <AddButtonBottom>
        <CreateDrawer isShowAddIcon={true} title="Create Assignment">
          <AssignmentForms grammarSubscription={grammarSubscription} />
        </CreateDrawer>
      </AddButtonBottom>
      {showDeleteWarning && (
        <WarningDialog
        ...
        {search ? (
          ...
          <>
            {rows.length > 0 ? (
              ...
              <CardView>
                <Instructions
                  message={Object.values(INSTRUCTIONS_STEPS.ASSIGNMENT)}
                />
              </CardView>
            )}
          </>
...
const mapDispatchToProps = (dispatch) => {
  return {
    DeleteAssignment: (ClasId, assId) =>
      dispatch(DeleteAssignment(ClasId, assId)),
    GetAssignment: (ClasId, PaginationValue) =>
      dispatch(GetAssignment(ClasId, PaginationValue)),

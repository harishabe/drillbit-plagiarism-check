...
const Students = ({
  ...
  const handleShow = (e, info) => {
    if (info?.title === "Add From List") {
      ...
      UploadFileDataClear();
      router.push({
        pathname: "/extream/instructor/addBulkStudent",
        query: { clasId: clasId, clasName: clasName },
      });
    }
  };
    ...
    <React.Fragment>
      {showDialogModal && (
        <>
          ...
          >
            <StudentInstitute classId={router.query.clasId} />
          </DialogModal>
        </>
      )}
      { showModal && (
        ...
        {search ? (
          ...
          <>
            {rows.length > 0 ? (
              ...
              <CardView>
                <Instructions
                  message={Object.values(INSTRUCTIONS_STEPS.STUDENT)}
                />
              </CardView>
            )}
          </>
...
const mapDispatchToProps = (dispatch) => {
  return {
    GetStudent: (ClasId, PaginationValue) =>
      dispatch(GetStudent(ClasId, PaginationValue)),

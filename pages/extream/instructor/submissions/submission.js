...
const columns = [
  ...
  { id: "action", label: "Actions", maxWidth: 135 },
];
...
const Submission = ({
  ...
  const handleUploadFile = () => {
    ...
    router.push({
      pathname: "/extream/instructor/uploadFile",
      query: router.query,
    });
  };
  ...
  const handleYesWarning = () => {
    DeleteSubmission(
      `classes/${clasId}/assignments/${assId}/submissions?paperId=${deleteRowData}`
    );
  ...
  const handleShow = (e, info) => {
    if (info?.title === "English") {
      router.push({
        pathname: "/extream/instructor/uploadFile/englishFile",
        query: router.query,
      });
    } else if (info?.title === "Non English") {
      router.push({
        pathname: "/extream/instructor/uploadFile/nonEnglishFile",
        query: router.query,
      });
    }
  };
...
const mapStateToProps = (state) => ({
  ...
  submissionData:
    state?.instructorMyFolders?.submissionData?._embedded?.submissionsList,

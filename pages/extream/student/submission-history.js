...
import { NonEnglishUploadIcon, EnglishUploadIcon } from "../../../assets/icon";
import { GetGrammarReport } from "../../../redux/action/common/Submission/SubmissionAction";
...
const SubmissionHistory = ({
  ...
  const handleShow = (e, info) => {
    if (info?.title === "English") {
      router.push({
        pathname: "/extream/student/uploadFile/englishFile",
        query: router.query,
      });
    } else if (info?.title === "Non English") {
      router.push({
        pathname: "/extream/student/uploadFile/nonEnglishFile",
        query: router.query,
      });
    }
  };
  ...
  return (
    <>
      {rows.length > 0 ? (
        ...
        <Instructions
          message={Object.values(INSTRUCTIONS_STEPS.STUDENT_SUBMISSION)}
        />
      )}

...
const Instructor = ({
  ...
  const handleYesWarning = () => {
    DeleteData(
      BASE_URL_EXTREM + END_POINTS.ADMIN_INSTRUCTOR_DELETE + deleteRowData,
      paginationPayload
    );
  ...
  const handleStatusWarning = () => {
    DeactivateData(
      BASE_URL_EXTREM +
        END_POINTS.ACTIVATE_DEACTIVATE_INSTRUCTOR +
        "/" +
        statusRowData.id +
        "/" +
        statusRowData.status,
      paginationPayload
    );
    setTimeout(() => {
      setStatusWarning(false);
  ...
  return (
    <React.Fragment>
      ...
      {editInstructor && (
        ...
        >
          <InstructorForm editData={editInstructorData} />
        </CreateDrawer>
      )}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    DeleteData: (url, paginationPayload) =>
      dispatch(DeleteData(url, paginationPayload)),

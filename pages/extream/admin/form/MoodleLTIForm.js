...
const MoodleLTIForm = ({
    ...
    const onSubmit = (data) => {
        if (editOperation) {
            ...
            ChangeConfig(BASE_URL_EXTREM + END_POINTS.ADMIN_MOODLE_LTI_INTEGRATION, detailedData);
        } else {
            let detailedData = {
                ...data, 'method': data.method?.name
            };
            LmsIntegration(BASE_URL_EXTREM + END_POINTS.ADMIN_MOODLE_LTI_INTEGRATION, detailedData);
        }
    };

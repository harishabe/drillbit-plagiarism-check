...
const folderSubmission = ({
  ...
  const TooltipContent = ({ value }) => (
    <>
      ...
      <div className={ classes.flex }>
        { " " }
        { value } <ArrowRightAltIcon fontSize="small" /> English{ " " }
      </div>
    </>
  );
  ...
  useEffect(() => {
    if (router.isReady) {
      folderSubmissionsFileData(
        BASE_URL_PRO +
        END_POINTS_PRO.USER_SUBMISSION +
        folderId +
        "/submissions",
        paginationPayload
      );
  ...
  const handleRefresh = () => {
    folderSubmissionsFileData(
      BASE_URL_PRO + END_POINTS_PRO.USER_SUBMISSION + folderId + "/submissions",
      paginationPayload
    );
  };
          ...
          submission?.language === "Cross-Language" ? (
            <div className={ classes.lang }>
              <div className={ classes.tooltipStyle }>
                <Tooltip
                  title={ <TooltipContent value={ submission.language1 } /> }
                  arrow
                >
  ...
  const handleYesWarning = () => {
    DeletefolderSubmissionData(
      BASE_URL_PRO +
      END_POINTS_PRO.USER_SUBMISSION +
      `${folderId}/submissions?paperId=${deleteRowData}`
    );
  ...
  const handleYesSaveWarning = () => {
    SaveToRepoBulk(
      BASE_URL_PRO +
      END_POINTS_PRO.USER_SUBMISSION +
      `${folderId}/submissions/repository?paperId=${saveRowData}`
    );
    setShowDeleteAllIcon(false);
    setTimeout(() => {
      setShowSaveIcon(false);
  ...
  const handleSubmissionDownloadYesWarning = () => {
    SubmissionReportDownload(
      ...
      END_POINTS_PRO.SIMILARITY_REPORT_SINGLE_DOWNLOAD +
  ...
  const handleShowAnalysisPage = (e, row) => {
    if (row?.language === FILE_LANGUAGE.REGIONAL) {
      if (row?.paper_id > 657600) {
        ...
        let url =
          BASE_URL_ANALYSIS + row.paper_id + "/" + row.d_key + "/" + token;
        windowOpen(url);
      } else {
  ...
  const handleShow = (e, info) => {
    if (info?.title === "English") {
      router.push({
        pathname: "/pro/user/uploadFile/englishFile",
        query: router.query,
      });
    } else if (info?.title === "Non English") {
      router.push({
        pathname: "/pro/user/uploadFile/nonEnglishFile",
        query: router.query,
      });
    } else if (info?.title === "Regional") {
      router.push({
        pathname: "/pro/user/uploadFile/regionalFile",
        query: router.query,
      });
    }
  };
            ...
            { isLoadingBulkDownload ? (
              ...
              <Tooltip title="Submission report bulk download" arrow>
                <StyledButtonIcon
                    ...
                    variant="outlined"
                    size="small"
                    onClick={ submissionBulkDownload }
                  >
                    <FileDownloadOutlinedIcon fontSize="small" />
                  </StyledButtonIcon>
                </Tooltip>
...
const mapStateToProps = (state) => ({
  ...
  folderSubmissionData:
    state?.submission?.folderSubmissionData?._embedded?.submissionsDTOList,
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    SubmissionReportBulkDownload: (url, requestPayload) =>
      dispatch(SubmissionReportBulkDownload(url, requestPayload)),
